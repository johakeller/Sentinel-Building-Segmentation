...
class SegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        building_cover=params.BUILDING_COVER,
        used_patches=None,
        dataset_size=None,
        train_path=params.DATASET_TRAIN,
        val_path=params.DATASET_VAL,
        test_path=params.DATASET_TEST,
        augmentations=None,
    ):
        ...
        if os.path.exists(os.path.join(self.dataset_path, f"{self.city}_data.mmap")):
            self.dataset = {
                "data": [
                    self.data_file_path,
                    (self.dataset_size, self.ch_num, self.patch_size, self.patch_size),
                ],
                "label": [
                    self.label_path,
                    (self.dataset_size, 1, self.patch_size, self.patch_size),
                ],
            }
    ...
    def __getitem__(self, item):
        ...
        data = np.memmap(
            self.dataset["data"][0], "float32", mode="r+", shape=self.dataset["data"][1]
        )
        ...
        # and from label
        label_data = np.memmap(
            self.dataset["label"][0],
            "float32",
            mode="r+",
            shape=self.dataset["label"][1],
        )
        ...
        all_out = np.stack(
            (data[item][3], data[item][0], data[item][1], data[item][2]), axis=0
        )
    ...
    def patch_check(self, patch_coord):
        ...
        """
        patch_corners = [
            (patch_coord[0], patch_coord[1]),
            (patch_coord[0] + 64, patch_coord[1]),
            (patch_coord[0], patch_coord[1] + 64),
            (patch_coord[0] + 64, patch_coord[1] + 64),
        ]  # [(y,x), (y+64, x), (y, x+64), (y+64, x+64)]

        # check if patch_corners fall inside on of the used patches
        for used_patch in self.used_patches:
            for point in patch_corners:
                if (
                    (point[0] >= used_patch[0]) and (point[0] <= used_patch[0] + 64)
                ) and (
                    (point[1] >= used_patch[1]) and (point[1] <= used_patch[1] + 64)
                ):
                    return False
    ...
    def dynamic_save(self, dataset):
        ...
        for key in dataset.keys():
            ...
            dataset_map = np.memmap(
                file_path, dtype="float32", mode="w+", shape=dataset[key].shape
            )
    ...
    def create_dataset(self):
        ...
        with open(images_path, "rb") as fp:
            ...
            bands_out = np.zeros(
                (self.dataset_size, 4, self.patch_size, self.patch_size)
            )
            labels_out = np.zeros(
                (self.dataset_size, 1, self.patch_size, self.patch_size)
            )
            ...
            for i in range(self.dataset_size):
                ...
                while keep_looking:
                    ...
                    r_ch = raw_data["R"][
                        patch_coord[0] : patch_coord[0] + self.patch_size,
                        patch_coord[1] : patch_coord[1] + self.patch_size,
                    ]
                    g_ch = raw_data["G"][
                        patch_coord[0] : patch_coord[0] + self.patch_size,
                        patch_coord[1] : patch_coord[1] + self.patch_size,
                    ...
                    buildings_ch = raw_data["Buildings"][
                        patch_coord[0] : patch_coord[0] + self.patch_size,
                        patch_coord[1] : patch_coord[1] + self.patch_size,
                    ]
                    ...
                    if self.mode == "training":
                        if self.cloud_check(nir_ch) and self.building_check(
                            buildings_ch
                        ):
                            ...
                            # set paramaters
                            self.used_patches.append(
                                patch_coord
                            )  # append coordinates of patch to used patches list
                            # set loop parameter
                            keep_looking = False
                    ...
                    # validation mode: check for cloud cover, also check if patch was not used in training, also check for min. building cover
                        ...
                        if (
                            self.cloud_check(nir_ch)
                            and self.patch_check(patch_coord)
                            and self.building_check(buildings_ch)
                        ):
                            # fill in output tensors: bands_out dim = (N,C,H,W)
                            bands_out[i][0] = r_ch
                            bands_out[i][1] = g_ch
