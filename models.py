    ...
    def __init__(self, bands, dropout_rate, batch_norm=True):
        ...
        if batch_norm:
            self.model = nn.Sequential(
                nn.Conv2d(self.channels_in, 32, kernel_size=3, padding=1),
                nn.ReLU(),
...
class UNet(nn.Module):
    ...
    class Encoder(nn.Module):
        def __init__(self, in_dimension, out_dimension, dropout_rate):
            ...
            self.conv_1 = nn.Conv2d(
                in_dimension, out_dimension, kernel_size=3, padding=1
            )
            self.conv_2 = nn.Conv2d(
                out_dimension, out_dimension, kernel_size=3, padding=1
            )
    ...
    class Decoder(nn.Module):
        def __init__(self, in_dimension, out_dimension, dropout_rate):
            ...
            self.up_conv = nn.ConvTranspose2d(
                in_dimension, out_dimension, kernel_size=2, stride=2
            )  # undo the max pooling to match the corresponding dimension on the other side of the U
            self.conv_1 = nn.Conv2d(
                2 * out_dimension, out_dimension, kernel_size=3, padding=1
            )
            self.conv_2 = nn.Conv2d(
                out_dimension, out_dimension, kernel_size=3, padding=1
            )
    ...
    def __init__(self, bands, channels_out, dropout_rate):
        ...
        self.output_layer = nn.Sequential(
            nn.Conv2d(64, channels_out, kernel_size=1), nn.Sigmoid()
        )
