...
def add_gaussian_noise(
    data_sample, probability=params.PROB, mean=params.GMEAN, sigma=params.STDDEV
):
    """
...
def salt_pepper_noise(data_sample, probability=params.PROB, sp_prob=params.SP_PROB):
    ...
    if rndm <= probability:
        ...
        coords_black = [
            [np.random.randint(0, img_height - 1), np.random.randint(0, img_width - 1)]
    ...
    if rndm <= probability:
        for band in bands:
            ...
            # just a single channel
                ...
                image = cv2.resize(
                    image, None, fx=f_zoom, fy=f_zoom, interpolation=cv2.INTER_CUBIC
                )
