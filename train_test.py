...
def write_results(output, file):
    ...
    with open(
        os.path.join(params.OUT_PATH, f"{file}.txt"), "a", encoding="utf-8"
    ) as file:
        file.write(output)
...
class Trainer:
    ...
    def __init__(
        ...
        self.model = model.to(
            params.DEVICE
        )  # initialize model after bands, because number of channels has to be adjusted
        self.train_output = train_output
        self.val_output = val_output
        self.learning_rate = lr
        self.optimizer = optim.Adam(
            model.parameters(), lr=self.learning_rate, weight_decay=weight_decay
        )
        if lr_scheduler:
    ...
    def training(self):
        ...
        for epoch in range(self.epochs):  # all epochs
            ...
            for (
            ...
            ):  # go through the dictionary train loader (for each city)

                # statistics
                prog_bar = tqdm(
                    total=len(dataloader.dataset),
                    desc=f"Training epoch {epoch+1}, {city}",
                    position=0,
                    leave=False,
                )
                avg_loss = 0.0
                ...
                for data in dataloader:  # go through data in each data loader
                    ...
                    loss = self.bce_f * self.criterion(
                    ...
                    all_predictions = torch.cat(
                        (
                            all_predictions,
                            (prediction > params.PRED_THRESHOLD).flatten().detach(),
                        )
                    )
            ...
            f1_tmp, accuracy_tmp, precision_tmp, recall_tmp = self.calculate_metrics(
    ...
    def validation(self):
        ...
        for (
        ...
        ):  # go through the dictionary validation loader (for each city)

            # statistics
            avg_loss = 0.0
            prog_bar = tqdm(
                total=len(dataloader.dataset),
                desc=f"{city} validation",
                position=0,
                leave=False,
            )
            ...
            with torch.no_grad():
                for i, data in enumerate(
                    dataloader
                ):  # go through data in each data loader
                    ...
                    ) + self.iou_f * self.iou(prediction, test_label)
                    avg_loss += loss.item()
                    ...
                    all_predictions = torch.cat(
                        (
                            all_predictions,
                            (prediction > params.PRED_THRESHOLD)
                            .int()
                            .flatten()
                            .detach(),
                        )
        ...
        # calculate, metrics, return f1 score
        f1, accuracy, precision, recall = self.calculate_metrics(
            all_labels, all_predictions
        )
    ...
    def test(self):
        ...
        with torch.no_grad():
            for data in dataloader:  # go through data in each data loader
                ...
                prediction = self.model.forward(test_input)  # forward pass
                loss = loss = self.bce_f * self.criterion(
                    prediction, test_label
                ) + self.iou_f * self.iou(prediction, test_label)
                avg_loss += loss.item()
                ...
                all_predictions = torch.cat(
                    (
                        all_predictions,
                        (prediction > params.PRED_THRESHOLD).int().flatten().detach(),
                    )
                )
