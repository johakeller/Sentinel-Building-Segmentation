...
class DataSplit:
    ...
    def __init__(
        ...
        self.val_loader = {
            city: DataLoader(
                dataset.SegmentationDataset(city, "validation", dataset_size=val_size),
                batch_size,
                shuffle=True,
                num_workers=0,
            )
        ...
        self.test_loader = DataLoader(
            dataset.SegmentationDataset(
                params.TEST_CITY, "test", dataset_size=test_size
            ),
            batch_size,
            num_workers=0,
        )  # only dataloader!
...
def train_apply_hyper(model_name=None):
    ...
    message = (
        f"Hyperparameters selected with F1 score of {max_score:.2f}: "
        + best_trainer.description
        + "\n"
    )
    print(message)
...
def augment_apply(model_name=None):
    ...
    augmentations_dict = {
        "additive Gaussian noise": augment.add_gaussian_noise,
    ...
    for descr, augmentations in augmentations_dict.items():
        data_split = DataSplit(
            augmentations=augmentations
        )  # init dataloader for train, valdiation, test
